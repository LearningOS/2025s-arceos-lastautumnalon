...
use memory_addr::{AddrRange, MemoryAddr, VirtAddr, VirtAddrRange};
...
fn sys_mmap(  
    ...
    get_file_like(fd).expect("get file err").read(&mut buf).expect("read to buf err");
    let mut ext = unsafe {&mut * (current().task_ext_ptr() as *mut TaskExt) };
    let mut uspace: axsync::MutexGuard<'_, axmm::AddrSpace> = ext.aspace.lock();
    let vaddr = uspace.find_free_area(uspace.base(), length.align_up_4k(),AddrRange { start: uspace.base(), end: uspace.end() }).expect("no free space!");
    uspace.map_alloc(vaddr,length.align_up_4k() , prot.into(), true).expect("alloc failed");
    uspace.write(vaddr, &buf).expect("write err");
